...
import { MediaProvider, MediaType, getSdk, } from './generated/graphql';
import { createRestartableClient } from './websocket';
...
export class PixAIClient {
    constructor(opt) {
        ...
        this.rawSdk = getSdk(this.sendRawRequest.bind(this));
    }
    get apiBaseUrl() {
    ...
    async generateImage(parameters, options = {}) {
        ...
        const $taskUpdated = this.$personalEvents.pipe(map(event => event.personalEvents?.taskUpdated), filter(taskUpdated => taskUpdated?.id === task.id));
        const subscription = onUpdate && $taskUpdated.subscribe(onUpdate);
        const $taskCompleted = $taskUpdated.pipe(filter(task => task.status === 'completed'));
    ...
    async uploadMediaFile(file) {
        ...
        const init = type === MediaType.Image && provider !== MediaProvider.S3
            ? {
                method: 'POST',
                body: formData,
            }
